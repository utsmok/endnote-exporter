...
|---|---|---|---|
| T001 | COMPLETED | Added MVP contract, support matrix, and privacy policy with served-mode baseline, ZIP-first intake, direct-folder progressive enhancement, and explicit browser-local attachment policy. | 2026-03-18 |
| T002 | COMPLETED | Added deterministic synthetic ZIP fixtures, normalized golden XML outputs, manifest metadata, and a parity harness using the current Python exporter as the oracle. | 2026-03-18 |
| T003 | COMPLETED | Added a worker-based `sql.js` feasibility spike under `web/scripts/`, validated representative approved fixtures and expected-failure classifications, and documented conservative continuation / wrapper-escalation thresholds. | 2026-03-18 |
| T004 | COMPLETED | Added conversion, parity, and attachment-policy specifications, including must-match semantics, documented browser-local divergences, and a metadata-only no-link attachment mode. | 2026-03-18 |
| T005 | COMPLETED | Added isolated Vite + TypeScript served-mode workspace with explicit app/adapters/core/worker boundaries and wired dedicated-worker bootstrap. | 2026-03-18 |
| T006 | COMPLETED | Added ZIP-first prepared-library normalization, structured archive failure classification, browser-input adapters, and fixture-backed normalization tests for approved `.enl + .Data` / `.enlp` shapes. | 2026-03-18 |
| T007 | COMPLETED | Added a typed `sql.js` worker adapter, structured EndNote reference/attachment query results, dedicated-worker query messaging, and fixture-backed worker-query validation for supported browser-local databases. | 2026-03-18 |
| T008 | COMPLETED | Remediated Wave 3 drift: fixed XML escaping/validity, corrected Python-parity mapping behavior, corrected local timestamp formatting, and added fixture-backed TypeScript parity coverage for supported fixtures. | 2026-03-18 |
| T009 | COMPLETED | Remediated structured export results and attachment semantics: preserved documented metadata-only attachment mode, added explicit attachment/skipped-record counts, and restored Python-style per-record tolerance with warnings. | 2026-03-18 |
